...
import {VenueListdata} from './venue-data';
import {DecimalPipe} from '@angular/common';
import {debounceTime, delay, switchMap, tap} from 'rxjs/operators';
import {SortColumn, SortDirection} from './venue-sortable.directive';

interface SearchResult {
  jobGrid: VenueListModel[];
  total: number;
}
...
function sort(jobGrid: VenueListModel[], column: SortColumn, direction: string): VenueListModel[] {
  if (direction === '' || column === '') {
    return jobGrid;
  } else {
...
function matches(country: VenueListModel, term: string, pipe: PipeTransform) {
...
export class JobGridService {
  getFormValidationErrors(contactForm: any): any {
  ...
  private jobLlist$ = new BehaviorSubject<VenueListModel[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let jobGrid = sort(VenueListdata, sortColumn, sortDirection);
