...
export class ListComponent implements OnInit {
  ...
  constructor(
    ...
    private amenitieservice: AmenitiesAPIService,
  ...
  ngOnInit(): void {
    ...
    this.jobList.subscribe((x) => {
      this.content = this.amenitiesData;
    });
  }
  ...
  async getAllAmenitiesList() {
    (
      await this.amenitieservice.getAmenitiesList(this.pageSize, this.pageNo)
    ).subscribe((res) => {
      this.amenitiesData = res;
      this.filteredUsers = [...this.amenitiesData];
    });
  }

  filter() {
    this.filteredUsers = [
      ...this.amenitiesData.filter((item) =>
  ...
  async deleteAmenitiesData(id, event) {
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
          if (result.value) {
            (await this.amenitieservice.deleteAmenities(id)).subscribe((res) => {
              this.getAllAmenitiesList();
          });
          } else if (
  ...
  onEdit(id) {
    this.router.navigate(["venue-amenities/amenities-form/" + id]);
  }
}
